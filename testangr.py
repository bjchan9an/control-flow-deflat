...
def get_relevant_nop_nodes(supergraph):
        global proj, pre_dispatcher_node, prologue_node, retn_node
        relevant_nodes = []
        nop_nodes = []
        for node in supergraph.nodes():
                bb = proj.factory.block(node.addr, node.size)
                if supergraph.has_edge(node, pre_dispatcher_node) and len(bb.capstone.insns) != 1:
                        relevant_nodes.append(node)
                elif bb.addr not in (prologue_node.addr, retn_node.addr, pre_dispatcher_node.addr):
                        nop_nodes.append(node)
        return relevant_nodes, nop_nodes

# Hook callee with retn
...
def symbolic_execution(start_addr, hook_addr = None, modify = None, inspect = False):
        global proj, relevants, modify_value
        if hook_addr != None:
                proj.hook(hook_addr, retn_procedure, length = 5)
        if modify != None:
                modify_value = modify
        ...
        sm.step()
        relevant_block_addrs = [x.addr for x in relevants]
...
start = 0x4009a0
proj = angr.Project(filename, load_options = {'auto_load_libs':False, 'main_opts':{'custom_base_addr':0}})
base_addr = proj.loader.main_object.mapped_base >> 12 << 12

# Parameter normalize is essential
cfg = proj.analyses.CFGFast(normalize=True)
target_func = cfg.functions.get(start)
supergraph = gh.to_supergraph(target_func.transition_graph)
...
print('relevant_blocks:', [hex(node.addr) for node in relevant_nodes])
...
flow = {}
for parent in relevants_without_retn:
        flow[parent] = []
modify_value = None
patch_instrs = {}
for relevant in relevants_without_retn:
        print("------------------relevant %#x------------------" % relevant.addr)
